    ...
    for key, item in live_open:
        ...
        try:
            ...
            olog(f"[ema_breakeven] ⚠ {symbol}: не удалось переставить SL в "
                 f"безубыток: {e}")
            _ema_event_log_write("breakeven_failed", symbol=symbol,
                                  profit_r=round(profit/risk, 3), error=str(e))
            continue
        olog(f"[ema_breakeven] ✓ {symbol}: профит {profit/risk:.2f}R — SL "
             f"переставлен в безубыток {_fmt_px(breakeven_px)} (был {_fmt_px(sl)})")
        _ema_event_log_write("breakeven_moved", symbol=symbol, direction=direction,
                              profit_r=round(profit/risk, 3), entry=entry,
                              old_sl=sl, new_sl=breakeven_px)
        ...
        with _ema_history_lock:
            ...
            if it2:
                # v3.6.10: ВАЖНО сохранить оригинальный SL ДО перезаписи —
                # _ema_diagnose_one считает risk=abs(entry-sl) от этого же
                # поля item["sl"]; если сюда просто затереть цену безубытка,
                # разбор такой сделки после закрытия станет мусорным (risk
                # ~0, verdict бессмысленный). orig_sl сохраняется только
                # один раз — при повторных проходах (не должно случаться
                # благодаря breakeven_done, но на всякий случай) не затрёт
                # уже сохранённый оригинал.
                it2.setdefault("orig_sl", it2["sl"])
                it2["sl"] = breakeven_px
...
EMA_EVENTS_FILE = os.path.expanduser("~/ema_events_diagnostics.jsonl")
...
<h1>&#9889; EMA Bounce Dossier <span style="font-size:12px;color:#8b949e;font-weight:normal">v__APP_VERSION__</span></h1>
