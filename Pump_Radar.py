...
def _stretch_liquidity_issue(candles, tf):
    ...
    дыры в истории торгов; (3) много баров без объёма вообще вперемешку с
    редкими реальными — не ловится (1)/(2), но именно такой паттерн дал
    ложный отрыв на живом примере (RLS_USDT, 15m): одиночный скачок среди
    мёртвых баров выглядит как настоящий отрыв, хотя это просто редкая
    сделка на тонком рынке, а не движение."""
    ...
    if len(notrade_window) >= STRETCH_LIQUIDITY_NOTRADE_WINDOW:
        notrade_n = sum(1 for c in notrade_window if not c.get("vol"))
        frac = notrade_n / len(notrade_window)
        if frac > STRETCH_LIQUIDITY_NOTRADE_MAX_FRAC:
            return (f"{notrade_n}/{len(notrade_window)} последних баров без объёма вообще "
