...
STRETCH_LIQUIDITY_NOTRADE_MAX_FRAC = 0.35   # если >35% баров в окне без объёма вообще — считаем неликвидом, независимо от временных разрывов
STRETCH_LIQUIDITY_NOTRADE_REL_THRESHOLD = 0.02   # v0.30.80: реальная находка на живом примере (FUTUON_USDT) — старая проверка требовала СТРОГО нулевой объём (vol==0), а "мёртвые" бары на тонком рынке обычно не буквально нулевые — одна крошечная сделка уже даёт vol>0 и проскакивает мимо проверки. Теперь бар считается "мёртвым", если его объём меньше 2% от МАКСИМАЛЬНОГО объёма в этом же окне — относительный порог, не абсолютный ноль, ловит и почти-нулевые бары
