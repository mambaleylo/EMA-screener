...
def _render_vol_anomaly_chart_png(candles, base_price, anomaly_t=None, signal_price=None):
    ...
    # v0.30.10: реальная, повторяющаяся путаница — объём и цена делили
    # ОДНУ И ТУ ЖЕ высоту графика (0 -> plot_h у обеих), поэтому высокий
    # по объёму столбик и высокая по цене точка визуально оказывались
    # рядом, хотя это совершенно не связанные величины — чистое совпадение
    # шкал. Объяснял это уже несколько раз, но путаница возникала снова —
    # значит дело не в объяснении, а в самом рендере. Теперь объём живёт
    # ТОЛЬКО в нижних 35% графика (отдельная суб-панель), цена — по-прежнему
    # на всю высоту для своего диапазона: даже максимальный по объёму
    # столбик физически не может дотянуться туда, где рисуется цена.
    VOL_PANEL_FRAC = 0.35

    def y_vol(v):
        vol_panel_h = plot_h * VOL_PANEL_FRAC
        vol_panel_top = pad_t + plot_h - vol_panel_h
        return vol_panel_top + (1 - v / vmax) * vol_panel_h
