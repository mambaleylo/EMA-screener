#!/usr/bin/env python3
"""
Pump Radar v0.30.64 (fork of EMA Invert Experiment v0.1.10, itself a fork of
...
STRETCH_DIAG_SCAN_WORKERS = 8   # v0.30.64: реальная находка по прямому вопросу "задержки по 5-6 минут" — 24 параллельных потока на публичный API без ключа, скорее всего, упирались в rate limit Gate.io (429), а на каждый неудачный запрос в _fetch_candles уже была логика до 5 попыток с паузой 5с между ними — если много монет разом ловят это, суммарная задержка утекает в минуты, ровно наоборот ожидаемому ускорению. Снижено до 8 — всё ещё на порядок быстрее последовательного обхода (v0.30.62), но заметно бережнее к лимитам API
