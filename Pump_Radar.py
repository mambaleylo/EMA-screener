#!/usr/bin/env python3
"""
Pump Radar v0.31.15 (fork of EMA Invert Experiment v0.1.10, itself a fork of
EMA Bounce Dossier v3.6.14 / SMC Optimizer v3.52.96)
- v0.31.15: откат по прямому запросу — "убери последний апдейт косячный,
  было же хорошо". STRETCH_REVERSE_STOP_PCT/TAKE_PCT вернул на 0.25/1.0,
  STRETCH_CONTINUATION_STOP_PCT — на 1.0 (было временно занижено до
  0.15/0.5 и 0.5 в v0.31.14). Честно: свежие данные на момент отката
  показали, что параметры сами по себе, скорее всего, не были причиной
  плохих результатов — старые и новые давали похожий, одинаково слабый
  винрейт у reverse (~16% на обоих), а "36% у continuation", что казался
  улучшением, был везением маленькой выборки (n=36), не сработавшим
  дальше. Инфраструктуру расширенного слежения (extended tracking для
  reverse/continuation MFE/MAE, v0.31.14) НЕ откатывал — это отдельное,
  рабочее улучшение точности данных, не связано с самими параметрами.
...
- v0.31.11: два связанных исправления. (1) По прямому запросу — "с такими
  ...
  4 дня) были откалиброваны под ШИРОКИЙ стоп 2% (v0.30.60) и с тех пор не
  пересматривались, хотя стоп давно сузили до 0.25% (v0.30.85/v0.31.0) —
  резолв почти всегда происходит в первые 1-3 свечи, не через дни.
  Сокращено до 3ч/5ч/10ч/36ч (расширенное — та же логика, ×3, теперь от
  меньшей базы: 9ч/15ч/30ч/4.5 дня вместо прежних до 12 дней).
  (2) По прямому вопросу "5мб не повлияют на сохранение нужных данных?" —
  реальный риск: наивная обрезка "самое старое первым" (v0.31.7) могла
  выкинуть запись в середине расширенного слежения, теряя именно то, что
  специально начали собирать дольше. Теперь обрезка сначала берёт только
  из уже ПОЛНОСТЬЮ завершённых записей (track_done и, если это fade —
  extended_track_done тоже) — активные трогает только если этого пула не
  хватило. С учётом (1) активных записей теперь и так намного меньше.
...
- v0.31.4: по прямому вопросу "почему всё стало плохо, как улучшить для
  ...
  различают слабо): 4h винрейт 21-27%/PnL -0.19...-0.36%, а 15m — винрейт
  0-7%/PnL -0.80...-1.00% (почти гарантированный слив). Быстрые ТФ дают
  "отрыв 5%+" слишком часто просто из шума, медленные — ближе к
  настоящему движению. Добавлен STRETCH_CONTINUATION_ALLOWED_TFS={"4h"}
  — алерт/автосделка для продолжения теперь только на 4h, остальные ТФ
  режутся (запись всё равно копится для наблюдения). Даже 4h пока в
  среднем в минусе, честно — это не полное решение, а снижение ущерба.
  Отдельно нашёлся сегмент 7-10% отрыва внутри 4h в честном плюсе
  (винрейт 34%/PnL+0.03%, n=35) — НЕ стал резать по нему сейчас, выборка
  тонковата, не хочу повторить историю с STRETCH_CONTINUATION_MIN_PCT
  (уже разворачивалась раньше) — стоит присматриваться дальше по мере
  накопления данных, не считать окончательным.
...
- v0.31.2: реальная находка по прямому вопросу "на сколько хватает 5 МБ
  ...
  объёма только от одной этой строки, даже когда вообще ничего не
  произошло (прикидка: только эта строка при 5с давала бы ~84 КБ/час,
  5 МБ хватило бы на ~60ч даже в абсолютной тишине, а с учётом остальных
  логов — реально заметно меньше). Теперь: если что-то найдено — пишем
  как раньше, каждый раз. Если пусто — редкий heartbeat раз в 5 минут,
  не на каждый цикл. Прямое следствие собственного фикса v0.30.99, не
  замеченное сразу.
...
- v0.31.0: по прямому вопросу — "статистика плохая, дать другие тейки и
  ...
  перебор показал avgPnL при 0.5% ушёл в минус (-0.016%), при 0.2-0.25%
  стабильно +0.087-0.089%; (2) добавлен фильтр по тренду для фейда
  (STRETCH_FADE_MAX_TREND_PCT=8.0, режет только алерт/автосделку, не
  запись) — реальный, но НЕИНТУИТИВНЫЙ разворот прежней находки
  (v0.30.90 показывал наоборот, сильный тренд лучше для фейда): сейчас
  |trend|<3%=винрейт 31%/PnL+0.055%, |trend|>=8%=14%/-0.246%. Направление
  зависимости меняется вместе с режимом рынка — константу стоит
  периодически пересматривать по свежим данным, не считать законом
  навсегда; (3) для continuation — ни тренд, ни объём СЕЙЧАС вообще не
  различают хорошие/плохие сетапы (везде одинаково плохо, 13-19%
  винрейт) — это не проблема "неправильного сегмента", похоже на то, что
  сама идея продолжения тренда сейчас не работает в принципе (рынок
  выглядит менее трендовым, чем раньше) — никакая перенастройка входа
  это не чинит, разумнее приостановить continuation-автоторговлю до
  восстановления, чем подбирать параметры под то, что не работает
  структурно. Фильтр по тренду подключён и к алерту (_fire), и к
  автоторговле (_stretch_autotrade_process_fresh) — раньше эти пути были
  разведены (v0.30.95), забыть один было бы реальным риском для денег.
- v0.30.99: по прямому вопросу — "откуда 45 секунд взялись, я думал
  просто гоняем список монет заново без остановки". Справедливо: due-
  фильтр (v0.30.68) уже гарантирует, что в большинстве циклов проверять
  почти нечего — цикл отрабатывает за доли секунды, а между циклами всё
  равно стоял безусловный time.sleep(45), даже когда сам цикл был почти
  мгновенным. Снижено до 5с. Не до нуля — единственная реальная плата:
  _fetch_all_symbols() (лёгкий тикер-запрос) дёргался бы совсем без
  паузы на каждой пустой итерации, разумнее оставить небольшой зазор,
  чем долбить биржу без остановки при полностью пустых циклах.
- v0.30.98: серьёзная реальная находка по прямому вопросу "даже с одним
  ТФ/EMA задержка 2-3 минуты" — разбивка по стадиям (очередь+лимит+
  запрос) показывала максимум ~1.5 минуты, а итоговая задержка в
  присланном экспорте доходила до 11-40 минут. Причина не в сети, а в
  двух местах внутри цикла обработки результатов скана, срабатывавших НА
  КАЖДОЕ совпадение: (1) seen_keys (дедуп-ключи, v0.30.96) пересобирался
  целиком из ВСЕЙ истории (до 5000 записей) при каждом новом совпадении;
  (2) _stretch_diag_save() перезаписывал ВЕСЬ файл на диск целиком. При
  всплеске из десятков/сотен совпадений это выполнялось внутри лока
  раз за разом и блокировало основной поток кумулятивно на минуты — не
  видно ни в одном из debug-полей, те меряют только параллельную часть
  (сетевые запросы), не последовательную обработку результатов. Теперь
  seen_keys собирается один раз ДО цикла обработки, а сохранение на диск
  — один раз В КОНЦЕ цикла. Алерты и автоторговля по-прежнему летят
  сразу по каждому совпадению — на них это никак не влияет.
...
STRETCH_DIAG_TRACK_WINDOW_SEC = {"15m": 3 * 3600, "30m": 5 * 3600, "1h": 10 * 3600, "4h": 36 * 3600}   # v0.31.11: реальный пересмотр по прямому запросу — "с такими стопами тейк/стоп должен быть уже буквально на следующей свече, для 15м многовато часов". Старые окна (6ч/12ч/24ч/4дня) были откалиброваны под ШИРОКИЙ стоп 2% (v0.30.60) и с тех пор не пересматривались, хотя стоп давно сузили до 0.25% (v0.30.85/v0.31.0) — при таком узком стопе резолв почти всегда происходит в первые 1-3 свечи, а не через дни
STRETCH_EXTENDED_TRACK_MULT = 3   # v0.31.8: по прямому запросу — насколько дольше следим за экстремумом ПОСЛЕ того, как фейд уже официально резолвился (стоп/тейк), чтобы честно оценить "а что было бы с разворотом в другую сторону" без искусственного обрыва данных слишком рано. v0.31.11: множитель тот же, но теперь применяется к заметно меньшей базе (см. STRETCH_DIAG_TRACK_WINDOW_SEC выше) — расширенное окно стало короче пропорционально
...
def _stretch_diag_save():
    try:
        ...
        if len(body) > STRETCH_DIAG_MAX_BYTES:
            # v0.31.11: по прямому вопросу "5мб не повлияют на сохранение
            # нужных данных?" — реальный риск: наивная обрезка "самое
            # старое первым" могла выкинуть запись, которая ещё СЕРЕДИНЕ
            # расширенного слежения (v0.31.8/9) — теряя именно те данные,
            # которые специально начали собирать дольше. Сначала обрезаем
            # только из уже ПОЛНОСТЬЮ завершённых (track_done и, если это
            # fade — extended_track_done тоже) — активные записи трогаем
            # только если этого пула не хватило, чтобы уложиться в лимит
            def _fully_done(r):
            ...
            # ужимаем done (от старых к новым), пока active + done не влезут
            lo, hi = 0, len(done)
            while lo < hi:
                mid = (lo + hi) // 2
                candidate = done[mid:] + active
                candidate.sort(key=lambda r: r["ts"])
                if len(json.dumps(candidate).encode()) <= STRETCH_DIAG_MAX_BYTES:
                    hi = mid
                else:
                    lo = mid + 1
            trimmed = done[lo:] + active
            trimmed.sort(key=lambda r: r["ts"])
            body = json.dumps(trimmed).encode()
            if len(body) > STRETCH_DIAG_MAX_BYTES:
                # активных записей самих по себе больше лимита — деваться
                # некуда, обрезаем и их тоже (от старых), это уже крайний случай
                ratio = STRETCH_DIAG_MAX_BYTES / len(body)
                keep_n = max(200, int(len(trimmed) * ratio * 0.95))
                trimmed = trimmed[-keep_n:]
                body = json.dumps(trimmed).encode()
                olog(f"[ema_stretch] ⚠ активных записей больше лимита {STRETCH_DIAG_MAX_BYTES} байт — пришлось обрезать и их")
        with _stretch_diag_lock:
            # v0.31.7: реальная находка — раньше обрезался только этот
            # ЛОКАЛЬНЫЙ trimmed (копия для записи на диск), а сам
            # _stretch_diag_records в памяти продолжал расти БЕЗ ГРАНИЦ —
...
_stretch_last_heartbeat_ts = 0   # v0.31.2: троттлинг для "пустых" циклов скана — раз в 5 минут, не на каждый цикл (был безусловным на каждый 5-секундный цикл — 9-кратный рост объёма лога после v0.30.99)
...
def _stretch_diag_scan_loop():
    ...
    while True:
        ...
        try:
            ...
            # v0.30.98: серьёзная реальная находка по прямому вопросу "даже с
            # одним ТФ/EMA задержка 2-3 минуты" — разбивка по стадиям
            # (очередь+лимит+запрос) показывала максимум ~1.5 минуты, а
            # итоговая задержка доходила до 11-40 минут. Причина не в сети —
            # в двух местах ниже, на КАЖДОЕ совпадение внутри цикла обработки
            # результатов: (1) seen_keys пересобирался целиком из ВСЕЙ
            # истории (до 5000 записей) — O(число_совпадений × размер
            # истории); (2) _stretch_diag_save() перезаписывал ВЕСЬ файл на
            # диск целиком. При всплеске из десятков/сотен совпадений это
            # выполнялось внутри лока десятки/сотни раз подряд и блокировало
            # основной поток кумулятивно на минуты — не видно ни в одном из
            # debug-полей, те меряют только параллельную часть. Теперь
            # seen_keys собирается ОДИН раз до цикла (не на каждое
            # совпадение), а сохранение на диск — ОДИН раз в конце цикла, а
            # не на каждое совпадение. Алерты и автоторговля по-прежнему
            # летят СРАЗУ по каждому совпадению — на них это не влияет.
            with _stretch_diag_lock:
            ...
            if added:
                _stretch_diag_save()   # v0.30.98: один раз в конце цикла, не на каждое совпадение — см. докстринг выше
            cycle_sec = round(time.time() - cycle_start, 1)
            _stretch_diag_last_cycle_sec = cycle_sec
            # v0.31.2: реальная находка по прямому вопросу "на сколько
            # хватает 5 МБ логов" — эта строка писалась БЕЗУСЛОВНО на
            # каждый цикл, а цикл теперь каждые 5с (было 45с, v0.30.99) —
            # 9-кратный рост объёма только от одной этой строки, даже
            # когда вообще ничего не произошло. Теперь: если что-то
            # реально нашлось — пишем как раньше, каждый раз. Если пусто —
            # редкий heartbeat (раз в 5 минут), не на каждый цикл
            global _stretch_last_heartbeat_ts
            if added:
            ...
            elif time.time() - _stretch_last_heartbeat_ts > 300:
                olog(f"[ema_stretch] скан завершён за {cycle_sec}с: без новых отрывов "
                     f"(задач {len(tasks)} из {len(symbols) * len(active_tfs)} возможных, "
                     f"всего в истории {len(_stretch_diag_records)})")
                _stretch_last_heartbeat_ts = time.time()
...
STRETCH_REVERSE_STOP_PCT = 0.25   # v0.31.15: откат по прямому запросу — "убери последний апдейт косячный, было же хорошо". Честно: свежие данные показали, что параметры сами по себе не были проблемой (старые 0.25/1.0 и новые 0.15/0.5 давали похожий, одинаково плохой результат ~16% винрейт) — но раз просили откатить, откатываю. v0.31.10: эксперимент "разворот" (LONG на слабом отрыве 1-2%, там же где фейд сейчас плохо работает)
STRETCH_REVERSE_TAKE_PCT = 1.0   # v0.31.15: откат, см. выше
...
STRETCH_CONTINUATION_STOP_PCT = 1.0   # v0.31.15: откат по прямому запросу — "убери последний апдейт косячный, было же хорошо". Честно: разбивка по времени показала, что "36% винрейт" на старых данных (n=36) был просто везением маленькой выборки — на большей выборке (n=245) стабильно плохо весь период, не резкий обвал только что; параметр сам по себе не был причиной. v0.30.84: узкий стоп (настоящий тренд не должен откатываться против нас сильно)
...
def _stretch_diag_maybe_alert_top_combo(new_recs):
    ...
    def _fire(rec, top, label):
        ...
        if stale:
            ...
            return
        if rec.get("strategy", "fade") == "fade" and rec.get("trend_pct") is not None \
                and abs(rec["trend_pct"]) >= STRETCH_FADE_MAX_TREND_PCT:
            # v0.31.0: по прямому вопросу "применить фильтры из логов" —
            # см. докстринг STRETCH_FADE_MAX_TREND_PCT — сейчас сильный
            # предшествующий тренд предсказывает плохой исход для фейда
            olog(f"[ema_stretch] {rec['symbol']} {top_tf} EMA{top_period} ({label}): "
                 f"алерт пропущен — слишком сильный тренд перед отрывом "
                 f"({rec['trend_pct']:+.1f}%, порог {STRETCH_FADE_MAX_TREND_PCT}%)")
            return
        if rec.get("strategy") == "continuation" and rec["tf"] not in STRETCH_CONTINUATION_ALLOWED_TFS:
            # v0.31.4: см. докстринг STRETCH_CONTINUATION_ALLOWED_TFS —
            # быстрые ТФ дают почти гарантированный слив у continuation
            olog(f"[ema_stretch] {rec['symbol']} {top_tf} EMA{top_period} ({label}): "
                 f"алерт пропущен — ТФ {rec['tf']} не в разрешённых для продолжения ({STRETCH_CONTINUATION_ALLOWED_TFS})")
            return
...
def _stretch_autotrade_process_fresh(fresh):
    ...
    for rec in fresh:
        ...
        if rec.get("strategy", "fade") == "fade" and rec.get("trend_pct") is not None \
                and abs(rec["trend_pct"]) >= STRETCH_FADE_MAX_TREND_PCT:
            # v0.31.0: тот же фильтр, что и у алерта в _fire — реальные
            # сделки должны учитывать ту же находку, не только уведомления
            olog(f"[stretch_autotrade] {rec['symbol']} {rec['tf']} EMA{rec['period']}: "
                 f"сигнал пропущен для автоторговли — слишком сильный тренд перед отрывом "
                 f"({rec['trend_pct']:+.1f}%, порог {STRETCH_FADE_MAX_TREND_PCT}%)")
            continue
        if rec.get("strategy") == "continuation" and rec["tf"] not in STRETCH_CONTINUATION_ALLOWED_TFS:
            olog(f"[stretch_autotrade] {rec['symbol']} {rec['tf']} EMA{rec['period']}: "
                 f"сигнал пропущен для автоторговли — ТФ не в разрешённых для продолжения ({STRETCH_CONTINUATION_ALLOWED_TFS})")
            continue
...
STRETCH_REF_STOP_PCT = 0.25   # v0.31.0: по прямому вопросу "статистика плохая, можно дать другие тейки/стопы?" — новый перебор на свежих 2323 решённых фейд-сигналах: при стопе 0.5% (было) avgPnL ушёл в минус (-0.016%), при 0.2-0.25% стабильно +0.087-0.089%. Ещё уже (0.05-0.15%) даёт похожий или чуть лучший результат, но это уже совсем близко к обычному спреду/проскальзыванию — 0.25% разумный компромисс, как и раньше при выборе 0.5% из бэктеста v0.30.85
