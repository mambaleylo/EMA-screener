#!/usr/bin/env python3
"""
EMA Invert Experiment v0.1.10 (fork of EMA Bounce Dossier v3.6.14)
- Разбор ema_invert_diagnostics.jsonl / ema_invert_signal_history.json за
  13.07 (100 закрытых сделок, winrate 58%, но баланс плоский/минус) — три
  независимых правки:
  1) _pick_best_ema_for_symbol: критерий отбора развёрнут с max bounce_rate
     на max break_rate (breaks/touches). corr(bounce_rate, live_pnl_pct)=
     -0.31 — чем надёжнее уровень исторически держит цену, тем хуже для
     ставки на пробой. Плюс два новых входных фильтра (EMA_INVERT_MAX_VOL_
     RATIO, EMA_INVERT_REJECT_CONFIRMED_PATTERN) — сильная реакция на
     касании (объём/свечной паттерн) статистически тоже означает "уровень
     держит", а не "будет пробит" (winrate 42% vs 66% и 45.5% vs 61.5% на
     соответствующих сегментах).
  2) EMA_INVERT_SAFETY_RR 1.4→1.1 и EMA_INVERT_TIME_STOP_BARS подняты
     ×1.5 — при winrate 58% старый RR=1.4 давал expectancy≈-0.01%/сделку
     (математика, не невезение); 78% time_stop-закрытий (14/18) дошли бы
     до TP в ближайшие бары, если бы не резались по времени раньше срока.
  3) Видимость PnL: новый fallback _gate_get_pnl_from_account_book (когда
     /position_close не даёт результата) + _ema_history_update_open —
     самый частый путь закрытия — раньше вообще не запрашивал live_pnl
     (3 из 17 "немых" закрытий были отсюда; остальные 14 — max_age_sec
     всё ещё не всегда успевает даже на 1800с).
...
- ЭКСПЕРИМЕНТАЛЬНЫЙ форк, не пушится в git, не участвует в авто-обновлении
  ...
  отдельный PORT — специально, чтобы можно было гонять одновременно с
  оригинальным EMA-screener на одном устройстве/аккаунте без коллизий.
- Идея (см. переписку): 18 записей ema_signal_diagnostics.jsonl показали
  post_sl_continued_against=13/18 (72%) — после срабатывания SL цена в
  следующие 20 баров ЕЩЁ РАЗ проходила минимум такое же расстояние в ту же
  (проигрышную для исходного сигнала) сторону, то есть уровень чаще
  пробивается трендом, чем даёт настоящий отскок. И среднее время до SL
  (5m≈3.1 бара, 15m≈2.7, 1h≈2) кратно меньше времени, которое в среднем
  нужно было бы для гипотетического TP — то есть если это движение
  торговать, а не отступать перед ним, прибыль (если она случится) приходит
  быстро.
- Правила игры инвертированы:
  1) Направление сделки — ПРОТИВОПОЛОЖНОЕ исходной логике отскока. Исходный
     бот на подтверждённом касании EMA ждёт отскок (long, если цена выше
     EMA); этот форк на ТОМ ЖЕ подтверждённом касании входит в сторону
     пробоя (short, если цена выше EMA — ставка на то, что уровень будет
     пробит, а не удержит).
  2) TP новой сделки = тот самый уровень (EMA ± sl_buf), который в
     оригинале был SL — см. EMA_SIGNAL_SL_ATR/_ema_check_symbol_signal.
     Формула не изменилась, изменился только смысл поля: "стоп по
     индикатору" оригинала здесь и есть цель этого форка.
  3) Жёсткого ценового SL, привязанного к стратегии, больше нет — основной
     выход по времени (см. п.4). На бирже всё равно выставляется широкий
     "предохранитель" (EMA_INVERT_SAFETY_RR × risk от TP) — не часть
     стратегии, а страховка на случай зависшего процесса/сети, чтобы
     позиция не осталась вообще без защиты на бирже.
  4) EMA_INVERT_TIME_STOP_BARS — если за это число баров ТФ сделки ни TP,
     ни safety-SL не сработали, сделка закрывается по рынку
     (_ema_invert_timestop_watchdog, рыночным ордером через
     _gate_close_position). Ориентир числа баров — те самые средние
     bars_to_sl из диагностики оригинала (округлено вверх, т.к. в выборке
     были и более медленные ТФ, которых тут больше нет).
  5) Таймфреймы ограничены EMA_LIVE_TFS <= 1h (1m/5m/15m/1h, без 4h/1d/1w) —
     по запросу, чтобы сделки были быстрыми и статистика по времени-до-TP
     копилась в разумный срок.
  6) breakeven-watchdog оригинала (перестановка SL в безубыток) отсюда
     убран — при отсутствии стратегического SL переставлять в безубыток
     нечего, эту роль теперь исполняет time-stop.
  7) Новый файл диагностики EMA_INVERT_DIAG_FILE
     (~/ema_invert_diagnostics.jsonl) — по каждой закрытой сделке этого
     форка: чем закрылась (tp/safety_sl/time_stop), сколько реально длилась
     (секунды и бары), сравнение с time_limit, MFE к TP, PnL%, и контекст
     входа (переиспользован из оригинала: RSI/ADX/DI/vol_ratio/ATR-перцентиль/
...
- EMA_INVERT_SAFETY_RR снижен с 2.0 до 1.4 (RR≈0.71 вместо 0.5) по разбору
  ...
  и один случайный SL (=2 TP по размеру) съедал профит с 12 плюсовых
  сделок разом. Убран assert, жёстко привязывавший этот параметр к
  EMA_SIGNAL_RR оригинала — теперь независим, тюнится по факту данных
  этого форка.
EMA Invert Experiment v0.1.3 (fork of EMA Bounce Dossier v3.6.14)
- ФИКС: _ema_invert_timestop_watchdog никогда не писал live_pnl/live_pnl_pct
  при закрытии по времени (в отличие от reconcile-пути для внешних
  закрытий). Разбор реального диагностического архива (24 закрытых live
  сделки) показал: 8 из 24 (33%) — все time_stop-исходы — вообще не имели
  зафиксированного $-результата, статистика по PnL/винрейту молча считала
  только tp/sl, теряя треть сделок. Теперь time_stop-закрытие так же
  запрашивает _gate_get_last_pnl и пишет live_pnl/live_pnl_pct/close_price
  из факта закрытия на бирже, как и внешние закрытия.
...
- v3.6.11: код-ревью по запросу — 4 фикса:
  1) [ema_touch]/[ema_breakeven] теперь дополнительно пишутся построчно
     (JSONL) в ~/ema_events_diagnostics.jsonl — раньше жили только в
     кольцевом буфере opt_state["logs"] (300-500 строк, обнуляется при
     рестарте), статистику за дни было не накопить.
  2) MAX_LEVERAGE=20 — верхний cap на плечо в _gate_open_position. Раньше
     leverage=risk_pct/sl_pct_val считался без ограничения сверху; вся
     защита от абсурдного плеча (риск: маленький ATR/погрешность float
     около risk≈0) держалась только на отказе Gate.io по лимиту контракта.
  3) /ema_signal_history_clear и /ema_dossier_start переведены с GET на
     POST — мутирующие действия не должны быть доступны обычным GET
     (случайный префетч браузера/WebView, ссылка в истории).
  4) _build_coin_dossier: недельный ТФ и recent_pump раньше молча пропадали,
     если бы функцию вызвали с timeframes без "1d" (сейчас нигде так не
     вызывается, но зависимость была скрытой) — дневные свечи теперь
     фетчатся отдельно, не как побочный эффект цикла. Плюс убран
     дублирующийся def log_message в HTTP Handler.
EMA Bounce Dossier v3.6.10 (fork of SMC Optimizer v3.52.96)
- v3.6.10: доводка диагностики под фиксы v3.6.9, до накопления новых данных:
  1) _ema_breakeven_watchdog переписывал item["sl"] в историю ценой
     безубытка — а _ema_diagnose_one считает risk=abs(entry-sl) от этого же
     поля. Сделка, закрытая по безубытку, дала бы risk≈0 и мусорный
     verdict. Watchdog теперь сохраняет item["orig_sl"] ДО перезаписи;
     diagnose использует orig_sl для risk/вердикта, а в саму запись лога
     добавлены "sl_hit_price" (реальная цена закрытия) и "breakeven_hit"
     (bool) — отдельный verdict "закрылась в безубыток" вместо того, чтобы
     такие сделки молча путались с настоящими стопами в статистике.
  2) Добавлена видимость для touch-подтверждения (v3.6.9, EMA_TOUCH_
     CONFIRM_SEC) — без неё нельзя было бы оценить, сколько мгновенных
     пробоев реально отсеивается и не слишком ли агрессивно фильтрует.
     Счётчик _ema_touch_confirm_stats (confirmed/aborted) + olog при
     каждой отмене подхода без подтверждения.
...
- v3.6.8: NUM_WORKERS — автоопределение по числу ядер устройства
  ...
  respawn-сценария старого SMC-оптимизатора (пул пересоздавался на каждый
  бар) — этот сценарий не существует в EMA-форке: единственный
  потребитель, _run_ema_dossier_scan, создаёт пул один раз за клик "скан"
  и явно закрывает по завершении. На разных устройствах (телефон/планшет
  с разным числом ядер) теперь своё значение без ручной подстройки через
  SMC_NUM_WORKERS. Явный SMC_NUM_WORKERS в окружении по-прежнему
  полностью переопределяет автовыбор.
...
def _check_version():
    ...
    _m2  = _re.search(r'EMA Invert Experiment v([\d.]+)', _src)
...
# v3.52.97: NUM_WORKERS раньше был cpu_count()-1 без верхней границы — но та
# опасность была специфична для СТАРОГО SMC-оптимизатора: там
# ProcessPoolExecutor(spawn) пересоздавался заново на каждое закрытие бара
# (~раз в tf_sec, десятки раз за сессию) — КАЖДЫЙ respawn на N воркерах это
# N холодных стартов python. На 8-ядерном телефоне это было 7 процессов на
# каждый respawn — вероятная причина постепенного исчерпания памяти и
# SIGKILL (LMK) на длинных прогонах. Поэтому тогда дефолт зафиксировали
# на 3 (безопасное число для ЛЮБОГО устройства) и вручную поднимали через
# SMC_NUM_WORKERS под конкретный телефон/планшет.
# v3.6.8: этот респавн-сценарий не существует в EMA-форке — SMC-оптимизатор
# вырезан в v3.6.5, единственный потребитель пула (_run_ema_dossier_scan)
# создаёт ProcessPoolExecutor ОДИН раз за клик "скан" и явно закрывает его
# по завершении (см. _shutdown_pool_safely) — фиксированный дефолт=3 больше
# не нужен для защиты, только недогружает более мощные устройства (у
# Anastasia — OnePlus-телефон и Huawei-планшет, ядер обычно разное
# количество). Теперь автоопределение: cpu_count()-1 (одно ядро оставляем
# ОС/интерфейсу, чтобы устройство не подвисало во время скана), отдельно
# на каждом устройстве своё значение без ручной подстройки. Явный
# SMC_NUM_WORKERS в окружении по-прежнему полностью переопределяет
# автовыбор (например, если на каком-то устройстве захочется меньше из-за
# памяти, а не ядер).
...
PORT         = 8766   # v0.1.0 (invert-fork): другой порт, чтобы можно было держать
                       # оригинальный EMA-screener (8765) запущенным параллельно
...
def _gate_get_last_pnl(symbol, max_age_sec=1800, fallback_since_ts=None):
    """v0.1.10: теперь тонкая обёртка. Основная логика (как раньше) — в
    _gate_get_last_pnl_from_position_close. Если она вернула None (запись
    старше max_age_sec или /position_close вообще ничего не отдал) И передан
    fallback_since_ts (обычно item["opened_at"] вызывающего кода) — пробуем
    _gate_get_pnl_from_account_book как второй источник, вместо того чтобы
    молча терять $-результат сделки (было 17/71 закрытий без live_pnl —
    см. таблицу разбора причин в комментарии к патчу v0.1.10)."""
...
# v0.1.0 (invert-fork): breakeven-watchdog оригинала здесь не нужен — у
# инвертированных сделок нет стратегического ценового SL, который имело бы
# смысл подтягивать (см. EMA_INVERT_SAFETY_RR ниже — единственный ценовой
# уровень защиты, это внешний предохранитель, не часть стратегии выхода).
# Вместо него — time-stop: если ни TP, ни safety-SL не сработали за
# EMA_INVERT_TIME_STOP_BARS баров ТФ сделки, закрываем по рынку.
# ── Ориентир числа баров — bars_to_sl из диагностики ОРИГИНАЛЬНОГО бота
# (ema_signal_diagnostics.jsonl, 18 записей): 5m≈3.1, 15m≈2.7, 1h≈2 бара.
# Округлено вверх (запас) и распространено на 1m (нет отдельных данных,
# консервативно тот же порядок, что 5m). Это ГИПОТЕЗА, а не откалиброванное
# значение — уточняется по мере накопления EMA_INVERT_DIAG_FILE самим этим
# форком (см. _ema_invert_diag_write_close и post_timestop_would_hit_tp).
...
# Широкий предохранитель на бирже (НЕ часть стратегии выхода — см. п.3 в
# шапке файла). RR здесь считается от TP-риска (|entry-tp|), а не наоборот:
# safety_sl = entry -/+ EMA_INVERT_SAFETY_RR * |entry-tp|.
# v0.1.4: было 2.0 (зеркалило EMA_SIGNAL_RR оригинала, TP:SL=1:2 от риска) —
# разбор диагностического архива (24 живых сделки) показал: при RR=0.5
# для простого безубытка нужен винрейт >=66.7%, запас минимальный, и один
# случайный SL (по размеру = 2 TP) съедал профит сразу с 12 плюсовых сделок
# (BEAT_USDT, -$0.69 против суммарных +$0.15 по всем TP). Снижено до 1.4
# (RR≈0.71) — TP:SL теперь примерно 1:1.4 от риска вместо 1:2, тот же
# винрейт даёт заметно более устойчивый эдж. Больше НЕ синхронизировано с
# EMA_SIGNAL_RR оригинала — это осознанное решение по факту данных этого
# форка, а не зеркалирование методички оригинала.
...
# сегментов, где эффект был наиболее выражен (см. коммент у
# _pick_best_ema_for_symbol с цифрами по каждому сегменту).
...
def _ema_invert_timestop_watchdog():
    """v0.1.0 (invert-fork): раз в цикл (см. _ema_signal_loop) проверяет все
    реально живые (live=True, status=open) сигналы этого форка — если с
    момента открытия (item['opened_at']) прошло больше time_limit_sec (см.
    _ema_invert_time_limit_sec), закрывает позицию по рынку через
    _gate_close_position и сам помечает историю status='time_stop' —
    ЗДЕСЬ, а не через _ema_reconcile_live_positions (который увидел бы это
    закрытие как 'внешнее' и не смог бы отличить принудительный time-stop
    от ручного закрытия/ликвидации по знаку pnl)."""
    with _ema_history_lock:
        state = _load_ema_history()
    live_open = [(k, v) for k, v in state["items"].items()
                 if v.get("status") == "open" and v.get("live")]
    ...
    for key, item in live_open:
        ...
        try:
            ...
            continue
        if not pos:
            continue   # уже закрыта другим путём — _ema_reconcile_live_positions разберётся
        ...
        # v0.1.3: раньше ЗДЕСЬ не писали live_pnl/live_pnl_pct вообще — в
        # отличие от reconcile-пути для внешних закрытий (см. ниже по коду),
        # time_stop-закрытие никогда не запрашивало _gate_get_last_pnl.
        # Итог: 8 из 24 реальных сделок (33%, весь time_stop-исход) не
        # имели зафиксированного $-результата в истории вообще — статистика
        # по винрейту/PnL считалась только по tp/sl, полностью пропуская
        # треть сделок. Теперь запрашиваем PnL так же, как reconcile.
        pnl_info = None
        try:
            # v0.1.10: fallback_since_ts=opened_at — см. _gate_get_pnl_from_account_book
...
# ─── Диагностика инвертированной логики (ema_invert_diagnostics.jsonl) ─────
# Отдельный файл от EMA_DIAG_FILE оригинала (другая схема: там разбор ПОЧЕМУ
# сработал обычный SL, здесь — как вела себя КАЖДАЯ закрытая сделка этой
# новой, инвертированной стратегии, любым исходом). Пишется не сразу в
# момент закрытия (для tp/safety_sl это можно было бы сразу, но для
# time_stop интересно ещё и увидеть, продолжила ли цена идти к TP уже ПОСЛЕ
# принудительного закрытия — не слишком ли туго время-стоп — поэтому все
# три исхода проходят через одну и ту же отложенную схему, для единообразия
# статистики).
EMA_INVERT_DIAG_FILE = os.path.expanduser("~/ema_invert_diagnostics.jsonl")
EMA_INVERT_DIAG_WAIT_BARS = 10   # меньше, чем EMA_DIAG_WAIT_BARS оригинала —
                                  # сделки этого форка сами по себе короче
...
def _ema_invert_diag_schedule(item):
    """Помечает запись как готовую к разбору диагностикой — сам разбор
    делает _ema_invert_run_diagnostics после EMA_INVERT_DIAG_WAIT_BARS."""
    item["diag_status"] = "pending"

def _ema_invert_diagnose_one(item):
    """Разбирает одну ЗАКРЫТУЮ (любым исходом) сделку инвертированной
    логики: сколько реально длилась, что случилось после закрытия (для
    time_stop — не ушла ли цена к TP вскоре после принудительного выхода,
    сигнал что лимит времени слишком тугой), MFE к TP за время жизни
    сделки. Возвращает dict-запись или None, если данных ещё недостаточно."""
    symbol, tf, ema_period = item["symbol"], item["tf"], item["ema_period"]
    direction = item["dir"]
    entry, tp, safety_sl = item["price"], item["tp"], item.get("sl")
    exit_reason = item["status"]           # "tp" | "sl" (=safety_sl) | "time_stop"
...
def _ema_invert_run_diagnostics():
    """Аналог _ema_run_diagnostics оригинала, но для ЛЮБОГО исхода (tp/sl/
    time_stop) закрытых сделок этого форка — см. _ema_invert_diagnose_one."""
    with _ema_history_lock:
        state = _load_ema_history()
...
def _gate_open_position(symbol, direction, entry_px, sl_px, tp_px, risk_pct, **kwargs):
    ...
    try:
        ...
        # v3.6.11: добавлен верхний cap (MAX_LEVERAGE) — см. комментарий у
        # константы выше. round() без ограничения мог в редком edge-case
        # (risk≈0 из-за маленького ATR/погрешности float) улететь в
        # неадекватное плечо ещё ДО отправки на биржу.
...
EMA_DOSSIER_FILE = os.path.expanduser("~/ema_invert_dossier_state.json")
...
# v0.1.4: раньше здесь был assert EMA_INVERT_SAFETY_RR == EMA_SIGNAL_RR —
# invert-fork сознательно зеркалил RR оригинала. С v0.1.4 это больше не
# так (см. коммент у EMA_INVERT_SAFETY_RR выше) — константы независимы,
# EMA_SIGNAL_RR здесь чисто для документации методички оригинала.
# v3.6.11: раньше leverage = risk_pct/sl_pct_val (см. _gate_open_position)
# считался БЕЗ верхней границы — вся защита от абсурдного плеча (если
# sl_pct_val случайно вышел маленьким: узкий ATR/малоликвидная монета/
# погрешность float около risk≈0) держалась только на том, что Gate.io сам
# отклонит запрос с превышением лимита контракта. Если конкретный контракт
# на бирже вдруг допускает более высокое плечо, чем ожидается — сделка
# реально открылась бы с ним. Явный потолок в коде — вторая, независимая
# от биржи линия защиты.
MAX_LEVERAGE = 20
...
def _build_coin_dossier(symbol, timeframes=None):
    ...
    # ── недельный ТФ (ресемплинг из дневных свечей) + pump-флаг ────────────
    # v3.6.11: раньше daily_candles_cache заполнялся ТОЛЬКО как побочный
    # эффект цикла выше (если "1d" оказывался среди timeframes) — если бы
    # эту функцию когда-нибудь вызвали с кастомным набором ТФ без "1d"
    # (сейчас нигде не вызывается, но параметр публичный), недельный ТФ и
    # recent_pump молча пропадали бы без единой строки в логе. Теперь
    # дневные свечи фетчатся отдельно, не завися от состава timeframes.
...
# не связана.
EMA_LIVE_FILE      = os.path.expanduser("~/ema_invert_live_signals.json")
...
EMA_HISTORY_FILE = os.path.expanduser("~/ema_invert_signal_history.json")
...
def _ema_reconcile_live_positions():
    ...
    for key, item in live_open:
        ...
        # v0.1.10: fallback_since_ts=item["opened_at"] — см. _gate_get_pnl_from_account_book
        pnl_info = _gate_get_last_pnl(item["symbol"], fallback_since_ts=item["opened_at"])
        # account_book-fallback не знает close_price (только денежный итог) —
        # в этом случае берём то же, что и при полном отсутствии pnl_info
        ...
        # v0.1.0 (invert-fork): в отличие от оригинала (диагностика только
        # для минусовых SL) — здесь разбираем ЛЮБОЙ закрытый исход, включая
        # внешний (см. _ema_invert_run_diagnostics/_ema_invert_diagnose_one).
        item["diag_status"]       = "pending"
        if pnl_info:
    ...
    for key, item in open_items:
        try:
            ...
            if outcome:
                ...
                # v0.1.0 (invert-fork): в отличие от оригинала (на разбор
                # ставились только минусовые SL) — здесь разбирается ЛЮБОЙ
                # исход (tp/sl/time_stop), см. _ema_invert_run_diagnostics.
                item["diag_status"] = "pending"
                # v0.1.10: раньше для этого (САМОГО ЧАСТОГО) пути закрытия
                # live_pnl вообще не запрашивался — единственные пути, где
                # live_pnl писался, это reconcile (внешние закрытия) и
                # timestop watchdog; обычное закрытие по TP/SL, детектируемое
                # прямо здесь по живой цене/свечам, оставалось "немым" по
                # деньгам (3 из 17 закрытий без live_pnl в архиве от 13.07 —
                # ровно отсюда). Только для live=True, чтобы не дёргать
                # Gate API на бумажных (не реальных) сигналах.
                if item.get("live"):
                    try:
                        pnl_info = _gate_get_last_pnl(item["symbol"], fallback_since_ts=item["opened_at"])
                    except Exception as e:
...
EMA_AUTO_TRADE_CFG_FILE = os.path.expanduser("~/ema_invert_auto_trade_cfg.json")
...
EMA_DIAG_FILE          = os.path.expanduser("~/ema_invert_signal_diagnostics.jsonl")
...
# v3.6.11: opt_state["logs"] (используется olog) — кольцевой буфер только в
# памяти процесса, максимум 300-500 строк и обнуляется при рестарте. Для
# событий [ema_touch]/[ema_breakeven] этого мало, если нужна статистика за
# дни (confirmed/aborted туча пишется в _ema_touch_confirm_stats, которая
# тоже только в памяти) — поэтому события ДОПОЛНИТЕЛЬНО пишутся построчно
# (JSONL) на диск, независимо от EMA_DIAG_FILE (там другая схема — разбор
# закрытых по SL сделок постфактум, а не сырые touch/breakeven события).
EMA_EVENTS_FILE = os.path.expanduser("~/ema_invert_events_diagnostics.jsonl")
_ema_events_lock = threading.Lock()

def _ema_event_log_write(event_type, **fields):
    """Пишет одну строку JSON в EMA_EVENTS_FILE: {"ts":..., "type":...,
    ...остальные поля события}. Ошибки записи не должны валить основной
    цикл — только лог в olog."""
...
def _ema_diagnose_one(item):
    ...
    v3.6.10: если по сделке сработал _ema_breakeven_watchdog, item["sl"] —
    это уже цена безубытка, а не оригинальный стоп; risk/verdict считаем от
    item["orig_sl"] (сохранён watchdog'ом ДО перезаписи), иначе анализ
    получил бы risk≈0 и бессмысленный вердикт. sl_hit_price — фактическая
    цена, по которой сделка реально закрылась (для отличия "закрылась в
    безубыток" от "закрылась по настоящему стопу" в самих логах)."""
    symbol, tf, ema_period = item["symbol"], item["tf"], item["ema_period"]
    direction = item["dir"]
    entry, tp = item["price"], item["tp"]
    sl_hit_price   = item["sl"]                       # цена, по которой реально закрылось
    sl             = item.get("orig_sl", sl_hit_price)  # исходный стоп — для risk/вердикта
    ...
    risk = abs(entry - sl)
    if risk <= 0:
        return None   # защита от деления на 0 (не должно случаться, но не рискуем)
    if direction == "long":
...
# v0.1.0 (invert-fork): по запросу — максимум 1h (без 4h/1d/1w), чтобы
# инвертированные сделки (выход по времени, см. EMA_INVERT_TIME_STOP_BARS)
# оставались быстрыми и статистика по ним копилась в разумный срок.
...
_ema_touch_confirm_stats = {"confirmed": 0, "aborted": 0}   # v3.6.10: для оценки, насколько
                                                              # часто фикс реально отсеивает
                                                              # мгновенные пробои (см. olog ниже)
...
def _ema_check_symbol_signal(symbol, pick):
    ...
    if not touched_now:
        ...
        if state == "pending":
            # v3.6.10: видимость работы фикса — без этого лога нельзя оценить,
            # сколько мгновенных пробоев реально отсеивается (и не слишком ли
            # агрессивно, если счётчик окажется намного больше "confirmed").
    ...
    if ladder is not None and ladder != expected:
        return None
    # v3.1 (оригинал): фильтр по тренду старшего ТФ — не открывать сигнал
    # ПРОТИВ дневного тренда. v0.1.0 (invert-fork): условие ЗЕРКАЛЬНО
    # развёрнуто относительно оригинала — там фильтровали по bounce-
    # направлению (direction), здесь нужно фильтровать по РЕАЛЬНОМУ
    # направлению сделки (trade_dir = противоположно direction, см. ниже) —
    # эта инвертированная стратегия как раз ставит на пробой ПО тренду, и
    # трейд имеет смысл отсеивать, если он идёт ПРОТИВ дневного тренда.
    # trade_dir="short" при direction=="long" → блокируем, если htf_trend
    # =="up" (шортить в дневной аптренд бессмысленно для гипотезы пробоя).
    # trade_dir="long" при direction=="short" → блокируем при htf_trend
    # =="down". permissive: нет данных — не блокируем (как и в оригинале).
    ...
    # v1.5 (оригинал): уровень за EMA — там, где у оригинала стоял SL отскока
    # (если пробили дальше этого — "отскок" не состоялся). Формула НЕ
    # менялась (EMA_SIGNAL_SL_ATR/atr_v — те же, что в оригинале).
    sl_buf = EMA_SIGNAL_SL_ATR * atr_v
    level = (ema_v - sl_buf) if direction == "long" else (ema_v + sl_buf)
    # ── v0.1.0 (invert-fork): инверсия правил игры ──────────────────────────
    # direction выше — это направление БОЛЬШИНСТВА исходной (отскок) логики,
    # оно всё ещё нужно было для проверки лесенки/HTF-фильтра/подтверждения
    # касания (см. код выше — не тронут, детекция касания осталась той же).
    # Сама СДЕЛКА теперь берётся в ПРОТИВОПОЛОЖНУЮ сторону — не "жду отскок
    # от уровня", а "ставлю на то, что уровень будет пробит и цена продолжит
    # идти туда же, куда уже идёт тренд". trade_dir — реальное направление
    # сделки; level (бывший SL оригинала) становится TP; SL здесь —
    # исключительно широкий предохранитель на бирже (EMA_INVERT_SAFETY_RR),
    # НЕ стратегический — реальный выход по времени, см.
    ...
    if trade_dir == "long":
        ...
        risk_to_tp = price - tp
        safety_sl = price + EMA_INVERT_SAFETY_RR * risk_to_tp
    # Защитная геометрическая проверка (тот же принцип, что и в оригинале) —
    # для long обязано быть safety_sl < price < tp, для short — tp < price <
    # safety_sl. risk_to_tp <= 0 значит уровень оказался не с той стороны от
    # цены (не должно случаться благодаря подтверждению касания выше, но не
    # рискуем) — сигнал не выдаём.
    if risk_to_tp <= 0:
        return None
    ...
    if trade_dir == "short" and not (tp < price < safety_sl):
        return None
    # v2.4 (оригинал): округление по значащим цифрам, не по фикс. кол-ву
    # знаков — критично для монет-микроцентовиков. Геометрия перепроверяется
    # ЕЩЁ РАЗ на округлённых значениях (см. коммент в оригинале выше).
    ...
    if trade_dir == "short" and not (tp_r < price < safety_sl_r):
        return None
    # v2.5 (оригинал): доп. контекст входа для диагностики — atr_v/dist_atr/
    # лесенка/touches. dist_atr/ladder/candle_pattern считаются от
    # ИСХОДНОГО direction (это описание самого касания EMA, не зависит от
    # того, в какую сторону эту информацию потом торгуют).
    ...
    return {
        ...
        "bounce_dir": direction,   # v0.1.0: направление ИСХОДНОЙ (отскок) гипотезы, для справки
...
def _ema_signal_loop():
    ...
    while True:
        try:
            ...
            _ema_invert_timestop_watchdog()  # v0.1.0 (invert-fork): закрытие по рынку, когда вышло время
            _ema_history_update_open()   # v1.7: проверяем открытые сигналы на TP/SL
            _ema_invert_run_diagnostics()   # v0.1.0 (invert-fork): разбор ЛЮБОГО закрытого исхода
        except Exception as e:
...
<title>EMA Invert Experiment</title>
<style>
...
<h1>&#9889; EMA Invert Experiment <span style="font-size:12px;color:#8b949e;font-weight:normal">v__APP_VERSION__</span></h1>
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
      try:
        if self.path == "/" or self.path == "/index.html":
            ...
            # v3.6.11: версия в шапке страницы — плейсхолдер, а не f-string/
            # .format() на всём EMA_HTML_PAGE, т.к. в CSS/JS внутри полно
            # литеральных { } (сломались бы при .format()).
    ...
    def do_POST(self):
        ...
        if self.path == "/alert_cfg":
            ...
            # v3.6.11: перенесено с GET на POST — мутирующие действия не
            # должны быть доступны обычным GET (риск случайного срабатывания
            # от префетча браузера/WebView, ссылки в истории и т.п.)
            with _ema_history_lock:
            ...
            # v3.6.11: перенесено с GET на POST — см. комментарий выше у
            # /ema_signal_history_clear.
