...
# safety_sl = entry -/+ EMA_INVERT_SAFETY_RR * |entry-tp|. По твоей правке —
# сохраняем исходное соотношение оригинала (EMA_SIGNAL_RR=2.0, было TP:SL=
# 2:1 от риска), только роли меняются местами: тейк — 1 часть (бывший SL),
# стоп — 2 части (бывший TP). Не произвольное число, а тот же RR, что и в
# оригинале, просто зеркально. Задано литералом (не EMA_SIGNAL_RR
# напрямую) — та константа определяется НИЖЕ по файлу, а эта секция идёт
# раньше; значения синхронизированы вручную, см. коммент у EMA_SIGNAL_RR.
EMA_INVERT_SAFETY_RR = 2.0   # = EMA_SIGNAL_RR оригинала, см. коммент выше
    ...
    for key, item in live_open:
        ...
        with _ema_history_lock:
            ...
            if it2:
                ...
                it2["close_price"] = live_price or it2.get("price")
...
assert EMA_INVERT_SAFETY_RR == EMA_SIGNAL_RR, (
    "v0.1.0 (invert-fork): EMA_INVERT_SAFETY_RR должен зеркалить "
    "EMA_SIGNAL_RR (см. коммент у обеих констант) — поправь оба, если "
    "меняешь один")
